...
#
SRC_FILES += Production_Code/tmp-xfakes-c.c
SRC_DIRS += Production_Code/Core/Src 
...
INCLUDE_DIRS += Production_Code/Core/Inc
INCLUDE_DIRS += Production_Code/Drivers/CMSIS/Device/ST/STM32F4xx/Include
INCLUDE_DIRS += Production_Code/Drivers/CMSIS/Include
INCLUDE_DIRS += Production_Code/Drivers/STM32F4xx_HAL_Driver/Inc
INCLUDE_DIRS += Production_Code/Drivers/CMSIS/DSP/Include
INCLUDE_DIRS += Production_Code/CANopenNode
...
ifeq ($(TEST), 0)
	COMPONENT_NAME = mock_cpputest
	MOCKS_SRC_DIRS += Mocks
	TEST_SRC_DIRS += tests/mocked_cpputests
	CPPUTEST_OBJS_DIR = test-obj/mocked_tests
	CPPUTEST_LIB_DIR = test-lib/mocked_tests
else
	COMPONENT_NAME = functionality_cpputest
	SRC_DIRS += Production_Code/Core/Src/mockable_src
	TEST_SRC_DIRS += tests/real_imp_cpputests
	CPPUTEST_OBJS_DIR = test-obj/realimp_tests
	CPPUTEST_LIB_DIR = test-lib/realimp_tests
endif
...
CPPUTEST_WARNINGFLAGS += -Wno-switch
CPPUTEST_WARNINGFLAGS += -Wno-pointer-to-int-cast
